...
import com.example.network.util.apiFlow
...
class MealRemoteDataSourceImpl @Inject constructor(
    ...
    override fun getMealsForTypes(type: String, number: Int): Flow<ApiResult<MealResponse>>  = apiFlow {
        mealApi.getMealsForTypes(type, number)
    }

    override fun getMealsForDiet(diet: String, number: Int): Flow<ApiResult<MealResponse>> = apiFlow {
